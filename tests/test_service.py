...
# Тестовые данные
SAMPLE_TEXT = """Machine learning is a subset of artificial intelligence (AI) that focuses on the development of algorithms and models that enable computers to learn from data. Unlike traditional programming where rules are explicitly defined, machine learning systems improve their performance through experience and data exposure. Deep learning, neural networks, and natural language processing are key technologies driving recent advances in AI."""

EXPECTED_SUMMARY = (
    "Machine learning enables computers to learn from data without explicit programming rules."
)
...
class TestLLMClient:
    ...
    async def test_llm_timeout(self):
        ...
        with (
            patch.object(client, "_call_api", side_effect=LLMTimeoutError("Timeout")),
...
class TestPipeline:
    ...
    async def test_pipeline_with_valid_request(self):
        ...
        with patch.object(pipeline.llm_client, "summarize", return_value=EXPECTED_SUMMARY):
    ...
    async def test_pipeline_llm_failure_fallback(self):
        ...
        with patch.object(pipeline.llm_client, "summarize", side_effect=LLMAPIError("API Error")):
            response = await pipeline.execute(request)
    ...
    async def test_pipeline_parse_error_fallback(self):
        ...
        with patch.object(pipeline.llm_client, "summarize", side_effect=ParseError("Parse Error")):
            response = await pipeline.execute(request)
